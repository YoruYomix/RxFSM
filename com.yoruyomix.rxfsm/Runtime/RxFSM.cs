...
namespace RxFSM
{
    public enum CallbackType { EnterState, ExitState, TickState, EnterStateAsync, ExitStateAsync, UnhandledTrigger }
    ...
    public sealed partial class FSM<TState> : IDisposable where TState : Enum
    {
        ...
        public Action<TState, TState, object> _testTransitionHook;
        ...
        public void Evaluate(object trigger)
        {
            if (_disposed || trigger == null || _deactivateCount > 0) return;
            if (_exitAsyncRunning)
            {
        ...
        internal bool TryEvaluate(object trigger)
        {
            ...
            if (_exitAsyncRunning) return false;
        ...
        public void TransitionTo(TState to)
        {
            ...
            if (_exitAsyncRunning) return;

            CancelAllFilterPipelines();
            CancelInterrupt();

            var prev = _current;
            ExecuteTransitionCore(prev, to, null);
        }
