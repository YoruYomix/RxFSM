...
namespace RxFSM.SourceGenerator
{
    ...
    public sealed class ActionTableGenerator : IIncrementalGenerator
    {
        ...
        private const string ExitAsyncAttr = "RxFSM.ExitStateAsyncAttribute";
            ...
            foreach (var member in symbol.GetMembers())
            {
                ...
                foreach (var attr in method.GetAttributes())
                {
                    ...
                    switch (name)
                    {
                        ...
                        case ExitAsyncAttr: kind = Kind.ExitAsync; break;
                        default: continue;
                    }
                    ...
                    if (error == null)
                    {
                        ...
                        else if (IsAsync(kind) && ctCount > 1)
                            error = $"'{method.Name}': more than one CancellationToken parameter.";
                        else if (!IsAsync(kind) && ctCount > 0)
                            error = $"'{method.Name}': CancellationToken is only allowed on [EnterStateAsync] or [ExitStateAsync].";
                    }
        ...
        private static void Emit(SourceProductionContext spc, Model model)
        {
            ...
            foreach (var b in model.Bindings)
                if (IsAsync(b.Kind)) { hasAsync = true; break; }
        ...
        private static string BindingStatement(Model model, Binding b)
        {
            ...
            switch (b.Kind)
            {
                ...
                case Kind.ExitAsync:
                {
                    ...
                    {
                        TriggerMode.Typed =>
                            $"cd.Add(fsm.ExitStateAsync<{b.TriggerFq}>(state, ({s} __p, {b.TriggerFq} __t, {CT} __ct) => {m}({args}).AsTask()));",
                        TriggerMode.Object =>
                            $"cd.Add(fsm.ExitStateAsync(state, ({s} __p, object __t, {CT} __ct) => {m}({args}).AsTask()));",
                        _ =>
                            $"cd.Add(fsm.ExitStateAsync(state, ({s} __p, {CT} __ct) => {m}({args}).AsTask()));",
                    };
                }
        ...
        private static bool IsAsync(Kind kind)
            => kind == Kind.EnterAsync || kind == Kind.ExitAsync;

        private enum Kind { Enter, Exit, Tick, EnterAsync, ExitAsync }
